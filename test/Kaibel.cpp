...
TEST(Kaibel, simpleInsertTest){
    ...
    Kaibel algo(graph, 1, 42, 1);

    EXPECT_TRUE(algo.query(0, 3));
    EXPECT_FALSE(algo.query(0, 4));
}

TEST(Kaibel, simpleInsertDeleteTest) {
    Kaibel algo(6, 1, 42, 1);
...
TEST(Kaibel, largeInsertTest) {
    count n = 100;

    std::mt19937 rng(42);
    std::uniform_int_distribution<int> dis(0, bigNum);

    Kaibel actual(n, 1, 42, 1);
    DTree expected(n);
...
TEST(Kaibel, largeInsertDeleteTest) {
    count n = 100;
