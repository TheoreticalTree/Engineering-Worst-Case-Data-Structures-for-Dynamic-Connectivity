...
TEST(GKKT, simpleInsertTest){
    ...
    GKKT algo(graph, 1, 42, 1);

    EXPECT_TRUE(algo.query(0, 3));
    EXPECT_FALSE(algo.query(0, 4));
}

TEST(GKKT, simpleInsertDeleteTest) {
    GKKT algo(6, 1, 42, 1);
...
TEST(GKKT, largeInsertTest) {
    count n = 100;

    std::mt19937 rng(42);
    std::uniform_int_distribution<int> dis(0, bigNum);

    GKKT actual(n, 1, 42, 1);
    DTree expected(n);
...
TEST(GKKT, largeInsertDeleteTest) {
    count n = 100;
