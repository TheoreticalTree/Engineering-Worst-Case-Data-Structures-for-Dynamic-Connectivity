...
TEST(Wang, simpleInsertTest){
    ...
    Wang algo(graph, 1, 42, 1);

    EXPECT_TRUE(algo.query(0, 3));
    EXPECT_FALSE(algo.query(0, 4));
}

TEST(Wang, simpleInsertDeleteTest) {
    Wang algo(6, 1, 42, 1);
...
TEST(Wang, largeInsertTest) {
    count n = 100;

    std::mt19937 rng(42);
    std::uniform_int_distribution<int> dis(0, bigNum);

    Wang actual(n, 1, 42, 1);
    DTree expected(n);
...
TEST(Wang, largeInsertDeleteTest) {
    count n = 100;
